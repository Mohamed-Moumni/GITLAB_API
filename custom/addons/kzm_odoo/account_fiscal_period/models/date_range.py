# -*- coding: utf-8 -*-

from odoo import models, fields, _

from dateutil.relativedelta import relativedelta
from odoo.exceptions import ValidationError


class DateRange(models.Model):
    _inherit = "date.range"

    period_ids = fields.One2many(comodel_name="date.range",
                                 inverse_name="fiscal_year_id",
                                 string=u"Périodes",
                                 required=False,
                                 domain=[('type_id.unit_of_time', '!=', 0)])
    fiscal_period = fields.Boolean("Is period", related='type_id.fiscal_period', store=True)
    fiscal_year_id = fields.Many2one(comodel_name="date.range", string=u"Exercice fiscal", required=False, )
    kzm_is_quinzaine = fields.Boolean("Is quizaine", related='type_id.kzm_is_quinzaine', store=True)
    previous_fiscal_year = fields.Many2one(comodel_name="date.range", string=u"Exercice fiscal précedant",
                                           required=False, )
    next_fiscal_year = fields.Many2one(comodel_name="date.range", string="Exercice fiscal suivant", required=False, )

    # @api.depends('type_id')
    # def is_fiscal_year(self):
    #     for record in self:
    #         is_fiscal_year = False
    #         if record.type_id.is_fisacal_year:
    #             is_fiscal_year = True
    #         record.is_fiscal_year = is_fiscal_year

    def create_period3(self):
        return self.create_period(3)

    def create_period(self, interval=1):

        period_obj = self.env['date.range']
        fiscal_period = self.env.ref('account_fiscal_period.fiscalperiod', raise_if_not_found=False)
        for fy in self:
            # fy.period_ids.unlink()
            ds = fy.date_start
            date_stop = fy.date_end

            while ds < date_stop:
                de = ds + relativedelta(months=interval, days=-1)
                if date_stop < de:
                    de = fy.date_stop
                if fiscal_period:
                    period_obj.create({
                        'name': ds.strftime('%m/%Y'),
                        'date_start': ds.strftime('%Y-%m-%d'),
                        'date_end': de.strftime('%Y-%m-%d'),
                        'type_id': fiscal_period.id,
                        'fiscal_year_id': fy.id,
                    })
                else:
                    raise ValidationError(_("Aucun type Période fiscale n'est trouvé"))
                ds = ds + relativedelta(months=interval)
        return True
